...
namespace MessagePassingComm
{
  ...
  public class Receiver : IMessagePassingComm
  {
    ...
    public bool openFileForWrite(string name, string toStorage)
    {
      try
      {
        string writePath = Path.Combine(toStorage, name);
  ...
  public class Sender
  {
    ...
    public bool postFile(string fileName, string fromStorage, string toStorage)
        {
      FileStream fs = null;
      long bytesRemaining;
      ...
      try
      {
        string path = Path.Combine(fromStorage, fileName);
        fs = File.OpenRead(path);
        bytesRemaining = fs.Length;
        channel.openFileForWrite(fileName, toStorage);
        while (true)
        {
  ...
  public class Comm
  {
    public Receiver rcvr = null;
    public Sender sndr = null;
    ...
    public bool postFile(string filename, string fromStorage, string toStorage)
    {
        return sndr.postFile(filename, fromStorage, toStorage);
    }
    }
  ///////////////////////////////////////////////////////////////////
  ...
  public class TestPCommService
  {
    ...
    public static bool testComm()
    {
      ...
      string toStorage = "../../../ServiceFileStore";
      string fromStorage = "../../../ClientFileStore";

            List<string> names = getClientFileList();
      foreach (string name in names)
      {
        TestUtilities.putLine(string.Format("transferring file \"{0}\"", name));
        bool transferSuccess = comm.postFile(name, fromStorage, toStorage);
